...
class ItemTest(models.Model):
    ...
    piscofins_cst = models.ForeignKey(PisCofinsCst, related_name='piscofins_cst_items_teste', on_delete=models.RESTRICT)
    ...
    user_created = models.ForeignKey(User, related_name='items_created_teste', on_delete=models.SET_NULL, null=True, blank=True)
    user_updated = models.ForeignKey(User, related_name='items_updated_teste', on_delete=models.SET_NULL, null=True, blank=True)
    
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['client', 'code'], name='unique_client_code_teste')
        ]

    def __str__(self):
        return self.description
    
class ImportedItemTest(models.Model):  
    STATUS_CHOICES = [
        (0, 'Produto Novo'),
