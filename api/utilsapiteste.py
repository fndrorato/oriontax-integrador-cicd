...
def delete_imported_items(client_id):
    try:
        ...
        ImportedItemTest.objects.filter(client=client_instance).delete()
        ...
        try:
            with transaction.atomic():
                ...
                if status_id == 1:
                    ...
                    ItemTest.objects.bulk_update(items_to_update, ['sequencial', 'estado_origem', 'estado_destino'])
...
def validateSelect(client_id, items_df, df, initial_log=None):
    ...
    num_to_update = ItemTest.objects.filter(
