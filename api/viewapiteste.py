...
from api.utilsapiteste import validateSelect, save_imported_logs
...
class ImportItemTestingView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            items_queryset = ItemTest.objects.filter(client=client).values(
...
class ClientItemTestingView(APIView):
    authentication_classes = [ClientTokenAuthentication]
    permission_classes = [IsAuthenticatedClient]
    ...
    def get(self, request):
        ...
        items_queryset = ItemTest.objects.filter(client=client, status_item__in=[1, 2])
        
        current_time = timezone.now()
        num_updated = ItemTest.objects.filter(
            status_item=1, 
            client=client
        ).update(
            status_item=2,
...
class ClientOneItemTestingView(APIView):
    authentication_classes = [ClientTokenAuthentication]
    permission_classes = [IsAuthenticatedClient]
    ...
    def get(self, request, code):
        ...
        items_queryset = ItemTest.objects.filter(client=client, code=code)  
